...
dotenv.config();
import { RPC_ENDPOINT,SWQOS_ENDPOINT, PRIVATE_KEY , GRPC_ENDPOINT , API_KEY, COMMITMENT_LEVEL, QUOTE_AMOUNT, X_API_KEY, BLOCK_ENGINE_URL, ISJITO, MINIMUM_PROFIT} from "./constants/constants";
...
async function getTokenPairaddresses(tokenAddress: string) {
  try {
    ...
    const [
      ...
      raydium_basePools,
      raydium_quotePools,
      raydiumcpmm_basePools,
      raydiumcpmm_quotePools,
      raydiumclmm_basePools,
      raydiumclmm_quotePools,
      orcawp_basePools,
      orcawp_quotePools,
    ...
    for (const acc of pumpamm_basePools) {
      addresses[`${tokenAddress}_pumpswap_${acc.pubkey.toBase58()}`]=[new BN(0),new BN(0),new BN(0),new BN(0),new BN(0)];
    }
    for (const acc of pumpamm_quotePools) {
      addresses[`${tokenAddress}_pumpswap_${acc.pubkey.toBase58()}`]=[new BN(0),new BN(0),new BN(0),new BN(0),new BN(0)];
    }
    for (const acc of raydium_basePools) {
      addresses[`${tokenAddress}_raydium_${acc.pubkey.toBase58()}`]=[new BN(0),new BN(0),new BN(0),new BN(0),new BN(0)];
    }
    for (const acc of raydium_quotePools) {
      addresses[`${tokenAddress}_raydium_${acc.pubkey.toBase58()}`]=[new BN(0),new BN(0),new BN(0),new BN(0),new BN(0)];
    }
    for (const acc of raydiumcpmm_basePools) {
      addresses[`${tokenAddress}_cpmm_${acc.pubkey.toBase58()}`]=[new BN(0),new BN(0),new BN(0),new BN(0),new BN(0)];
    }
    for (const acc of raydiumcpmm_quotePools) {
      addresses[`${tokenAddress}_cpmm_${acc.pubkey.toBase58()}`]=[new BN(0),new BN(0),new BN(0),new BN(0),new BN(0)];
    }
    for (const acc of raydiumclmm_basePools) {
      addresses[`${tokenAddress}_clmm_${acc.pubkey.toBase58()}`]=[new BN(0),new BN(0),new BN(0),new BN(0),new BN(0)];
    }
    for (const acc of raydiumclmm_quotePools) {
      addresses[`${tokenAddress}_clmm_${acc.pubkey.toBase58()}`]=[new BN(0),new BN(0),new BN(0),new BN(0),new BN(0)];
    }
    for (const acc of orcawp_basePools) {
      addresses[`${tokenAddress}_orcawp_${acc.pubkey.toBase58()}`]=[new BN(0),new BN(0),new BN(0),new BN(0),new BN(0)];
    }
    for (const acc of orcawp_quotePools) {
      addresses[`${tokenAddress}_orcawp_${acc.pubkey.toBase58()}`]=[new BN(0),new BN(0),new BN(0),new BN(0),new BN(0)];
    }
    for (const acc of meteoradlmm_basePools) {
      addresses[`${tokenAddress}_dlmm_${acc.pubkey.toBase58()}`]=[new BN(0),new BN(0),new BN(0),new BN(0),new BN(0)];
    }
